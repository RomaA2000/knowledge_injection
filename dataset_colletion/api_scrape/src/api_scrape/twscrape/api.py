...
from .constants import GQL_FEATURES, GQL_URL, SEARCH_TIMELINE_GQL_PATH
...
class API:
    ...
    async def search(self, q: str, limit: int = -1, kv: object = None) -> AsyncIterator[Tweet]:
        twids = set()
    ...
    # (from:BBCWorld, OR from:ReutersWorld) -filter:replies
    async def search_from_accounts(self, accounts: List[str], limit: int) -> AsyncIterator[Tweet]:
        """
        Возвращает генератор с твитами, при этом:
        1) твиты возвращает
        2) ретвиты не возвращает
        3) реплаи возвращает
        4) quotes возвращает, но в UI поиска твитов их нет
        """
        if len(accounts) == 0:
            return

        accounts_joined = " OR ".join(map(lambda acc_name: f"from:{acc_name}", accounts))
        query: str = f'({accounts_joined})'
        async for i in self.search(q=query, limit=limit):
            yield i
