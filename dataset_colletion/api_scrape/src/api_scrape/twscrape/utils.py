...
def first_index_of(predicate: Callable[[T], bool], source: typing.List[T]) -> int:
    """
    Находит индекс первого элемента подходящего под предикат
...
def find_single(predicate: Callable[[T], bool], source: typing.Iterable[T]) -> typing.Optional[T]:
    """
        Находит элемент по предикату или возвращает None, если его нет.
        Кидает исключение если элементов удовлетворяющих предикату несколько
        """
    filtered = [element for element in source if predicate(element)]
    if len(filtered) == 0:
        return None
    if len(filtered) > 1:
        raise ValueError(f"Many elements found by predicate in source: {list(source)}, while only 1 expected")

    return filtered[0]


def last_or_null(source: typing.List[T]) -> typing.Optional[T]:
    if len(source) == 0:
        return None
    return source[-1]


def find_single_or_throw(predicate: Callable[[T], bool], source: typing.Iterable[T]) -> T:
    """
    Находит элемент по предикату.
    Кидает исключение если элементов удовлетворяющих предикату нет или их несколько
    """
    filtered = [element for element in source if predicate(element)]
    if len(filtered) == 0:
        raise ValueError(f"No elements found by predicate in source: {list(source)}")
    if len(filtered) > 1:
        raise ValueError(f"Many elements found by predicate in source: {list(source)}, while only 1 expected")
...
def throw_if_not_utc_zone(timestamp: datetime):
    if timestamp.tzinfo is not timezone.utc:
        raise ValueError(f"Timestamp {timestamp} is not in UTC timezone")
