...
async def login(acc: Account, fresh=False, suppress_http_status_error=True, close_client=False) -> Account:
    log_id = f"{acc.username} - {acc.email}"
    if acc.active and not fresh:
        logger.warning(f"account already active {log_id}")
        return acc

    logger.info(f"logging in {log_id}")
    ...
    while True:
        ...
        try:
            ...
            if not suppress_http_status_error:
                raise Exception(e)
    ...
    if close_client:
        await client.aclose()
        logger.debug(f"client for {log_id} closed")
